...
npm i @netlify/gotrue-js lucide-react tailwindcss postcss autoprefixer | Out-Null

# --- netlify.toml
Write-Text -Path "netlify.toml" -Text @"
[build]
...
# --- serverless search function (CommonJS for CRA/Netlify)
Write-Text -Path "netlify/functions/search.js" -Text @"
...
# --- postcss & tailwind config (safe overwrite)
Write-Text -Path "postcss.config.js" -Text @"
module.exports = { plugins: { tailwindcss: {}, autoprefixer: {} } };
"@

Write-Text -Path "tailwind.config.js" -Text @"
...
# --- ensure Tailwind entry
if (Test-Path "src/index.css") {
  $css = Get-Content "src/index.css" -Raw
  if ($css -notmatch "@tailwind base") {
    Write-Text -Path "src/index.css" -Text "@tailwind base;`r`n@tailwind components;`r`n@tailwind utilities;`r`n:root { color-scheme: light; }`r`n"
  }
} else {
  Write-Text -Path "src/index.css" -Text "@tailwind base;`r`n@tailwind components;`r`n@tailwind utilities;`r`n:root { color-scheme: light; }`r`n"
}

# --- Identity widget injection
if (Test-Path "public/index.html") {
  $html = Get-Content "public/index.html" -Raw
  if ($html -notmatch "identity\.netlify\.com") {
    $updated = $html -replace "</head>", "    <script src=""https://identity.netlify.com/v1/netlify-identity-widget.js""></script>`r`n</head>"
    Write-Text -Path "public/index.html" -Text $updated
  }
}

# --- src/lib
Write-Text -Path "src/lib/auth.js" -Text @"
...
Write-Text -Path "src/lib/utils.js" -Text @"
export function cn(...classes) { return classes.filter(Boolean).join(' '); }
"@

# --- UI primitives
Write-Text -Path "src/components/ui/button.jsx" -Text @"
...
Write-Text -Path "src/components/ui/input.jsx" -Text @"
import { cn } from '../../lib/utils';
...
Write-Text -Path "src/components/ui/card.jsx" -Text @"
import { cn } from '../../lib/utils';
...
Write-Text -Path "src/components/ui/dialog.jsx" -Text @"
...
Write-Text -Path "src/components/ui/label.jsx" -Text @"
import { cn } from '../../lib/utils';
...
# --- App components
Write-Text -Path "src/components/Navbar.jsx" -Text @"
...
Write-Text -Path "src/components/AuthModal.jsx" -Text @"
...
Write-Text -Path "src/components/CaseCard.jsx" -Text @"
...
Write-Text -Path "src/components/Search.jsx" -Text @"
...
# --- App.js (overwrite only if CRA default or missing marker)
$replaceApp = $true
if (Test-Path "src/App.js") {
  $app = Get-Content "src/App.js" -Raw
  if ($app -notmatch "Search U.S. civil rights case law") { $replaceApp = $true } else { $replaceApp = $false }
}

if ($replaceApp) {
Write-Text -Path "src/App.js" -Text @"
import { useEffect, useState } from 'react';
...
Write-Host "✅ Files written. You can now build locally with 'npm run build'."
